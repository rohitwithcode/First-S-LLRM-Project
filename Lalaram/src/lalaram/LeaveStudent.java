...
public class LeaveStudent extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					LeaveStudent frame = new LeaveStudent();
	...
	public LeaveStudent() {
		...
		JLabel lblNewLabel = new JLabel("Apply Leave(Student)");
		...
		textFieldRollNumber.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(rollNumber.isEmpty()) {
					JOptionPane.showMessageDialog(null, "please enter roll number for leave", "ERROR", JOptionPane.ERROR_MESSAGE);
				}
				else {
				try {
					...
					if(!rollNumberFound) {
						JOptionPane.showMessageDialog(null, "roll number not found in our database.Please input valid roll number");
					}
		...
		textFieldRollNumber.setBounds(166, 115, 172, 20);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(rollNumber.isEmpty()) {
					...
					try {
						...
						String query="insert into studentleave values(rollno,name,fathername,leavedate,leaveduration)";
						
			            String insertQuery = "INSERT INTO studentleave (rollno, name, fathername, leavedate, leaveduration) VALUES (?, ?, ?, ?, ?)";
		...
		JLabel lblNewLabel_1_1_3 = new JLabel("Student Name");
